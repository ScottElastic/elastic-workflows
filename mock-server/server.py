#!/usr/bin/env python3
"""
LLM-backed mock vendor API server. Uses Amazon Bedrock to synthesize realistic,
context-aware vendor responses on demand for SOAR workflow testing.
...
Responses are cached — identical requests are served from cache and never hit
Bedrock twice.

Env vars:
    BEDROCK_MODEL_ID      default: us.anthropic.claude-opus-4-8
    BEDROCK_REGION        default: us-west-2
    S3_CACHE_BUCKET       S3 bucket name for response cache (falls back to CACHE_DIR if unset)
    S3_CACHE_PREFIX       S3 key prefix for cache objects (default: cache/)
    CACHE_DIR             local cache directory when S3_CACHE_BUCKET is not set (default: ./cache)
    API_KEY               shared secret required in X-Api-Key header (auth skipped if unset)
    PORT                  default: 8080
...
Local dev:
    ...
    # With API key:
    API_KEY=mysecret python3 server.py
    curl -H "X-Api-Key: mysecret" http://localhost:8080/health

Smoke tests:
    curl -H "X-Api-Key: $API_KEY" http://localhost:8080/api/v3/ip_addresses/192.168.1.1 | jq
    curl -H "X-Api-Key: $API_KEY" http://localhost:8080/api/v3/ip_addresses/185.220.101.47 | jq
    curl -H "X-Api-Key: $API_KEY" http://localhost:8080/v3/community/8.8.8.8 | jq
    curl -H "X-Api-Key: $API_KEY" http://localhost:8080/api/now/table/incident | jq

Cache:
    # Upload local cache to S3 before first cloud deployment:
    aws s3 sync ./scripts/test-harness/cache s3://<bucket>/cache/
    # Or evict a specific entry:
    aws s3 rm s3://<bucket>/cache/<key>.json
"""
...
S3_CACHE_PREFIX = os.environ.get("S3_CACHE_PREFIX", "cache/")
...
# Epoch window for *_date integer fields. Lower bound is 2025-01-01.
# Upper bound derived from TODAY so it stays in sync as the server runs.
_today_dt = datetime.datetime.strptime(TODAY, "%Y-%m-%d").replace(tzinfo=datetime.timezone.utc)
_TODAY_EPOCH = int(_today_dt.timestamp())
EPOCH_LOWER = 1735689600   # 2025-01-01 00:00 UTC
EPOCH_UPPER = _TODAY_EPOCH

# Pre-computed scenario epochs. The LLM's epoch arithmetic for post-2024 dates
# is unreliable — it consistently lands one year early. Compute here and pass
# them verbatim so the model can copy them into *_date integer fields.
...
# fetches results by that id. Without correlation the LLM has nothing to
# anchor on. We keep a small in-memory map from (kind, id) → originating
# request body. Bounded at REQUEST_TRACE_LIMIT to keep memory steady.
...
def _assess_indicator(indicator: dict) -> str:
    ...
    if value in (CANONICAL["sha256"], CANONICAL["md5"], CANONICAL["sha1"]):
        return (
    ...
    if value.lower() in (
        CANONICAL["user"].lower(), CANONICAL["second_user"].lower(), CANONICAL["user_sam"].lower()
    ):
        return (
...
def cache_load(key: str):
    if s3:
        try:
            obj = s3.get_object(Bucket=S3_CACHE_BUCKET, Key=f"{S3_CACHE_PREFIX}{key}.json")
            return json.loads(obj["Body"].read())["payload"]
...
def cache_save(key: str, payload, meta: dict):
    data = json.dumps({"meta": meta, "payload": payload}, indent=2)
    if s3:
        s3.put_object(
            Bucket=S3_CACHE_BUCKET,
            Key=f"{S3_CACHE_PREFIX}{key}.json",
...
def synth(method: str, path: str, query: str, body: bytes) -> dict:
    ...
    effective_body = correlated.get("origin_body", body_str)
    effective_path = correlated.get("origin_path", path)

    indicator = _extract_indicator(effective_path, query, effective_body)
    assessment = _assess_indicator(indicator)
    anchored = _request_touches_scenario(effective_path, query, effective_body)
...
def _defender_machine_action(method, path, query, body):
    ...
    return 201, {
        ...
        "status": "Pending",
        "creationDateTime": f"{TODAY}T13:00:00Z",
    }
...
HARDCODED_HANDLERS = [
    ...
    (re.compile(r"^/api/machines/[^/]+/(isolate|unisolate)$"), ("POST",),  _defender_machine_action),
]
...
class Handler(BaseHTTPRequestHandler):
    def _serve(self, method: str):
        ...
        # Health check — no auth required; used by App Runner and load balancers.
        ...
        # API key check. Skip if API_KEY is not configured (local dev convenience).
        if API_KEY:
        ...
        if hc is not None:
            ...
            if payload is None:
                ...
                self.end_headers()
                print(f"[hardcd] {method} {parsed.path} -> {status_code} (no body)")
                return
            ...
            print(f"[hardcd] {method} {parsed.path} -> {status_code} {len(out)}B")
            return
        ...
        body_str = body.decode("utf-8", "replace") if body else ""
        _trace_creation(method, parsed.path, body_str, payload)

        out = json.dumps(payload).encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(out)))
        self.send_header("X-Mock-Source", source.strip())
        self.send_header("X-Mock-CacheKey", key)
        self.end_headers()
        self.wfile.write(out)
        print(f"[{source}] {method} {parsed.path} -> {len(out)}B")
...
def preflight():
    try:
        ...
        print(f"ERROR: Bedrock preflight failed: {e}", file=sys.stderr)
        print(f"  Model: {MODEL_ID}  Region: {REGION}", file=sys.stderr)
        print("  Override with BEDROCK_MODEL_ID / BEDROCK_REGION env vars.", file=sys.stderr)
        sys.exit(1)


def main():
    cache_loc = f"s3://{S3_CACHE_BUCKET}/{S3_CACHE_PREFIX}" if S3_CACHE_BUCKET else str(_cache_dir.resolve())
    print(f"LLM mock server")
    print(f"  port:   {PORT}")
    print(f"  model:  {MODEL_ID}")
    print(f"  region: {REGION}")
    print(f"  cache:  {cache_loc}")
    print(f"  auth:   {'X-Api-Key required' if API_KEY else 'DISABLED (set API_KEY to enable)'}")
    print("  verifying Bedrock creds...", end=" ", flush=True)
    preflight()
    print("ok\n")
    print("Ready. Logs: [bedrock] = fresh LLM call, [cache  ] = served from disk/S3\n")
